package class5;

public class Nestedif {

	public static void main(String[] args) {
		
		/*
		 * // create a java program and store values of mortgage rate and mortgage
		 * price. First, program should check if rate is higher than 4.5 user will not buy a
		 * house, otherwise user will consider buying. Once user decides to buy a house, 
		 * if price of the house is larger than 20,0000 than user will take a loan,
		 * otherwise user will pay cash.
		 */

		double rate =4.5;
		int mortgage =20000;
		
		if(rate>4.5) {
			System.out.println("Rate is high customer will not buy a house");
		}else {
			System.out.println("");
		}
	}

}
